    ...
    <Layout>
      ...
      <>
        ...
        {aboutData.map((section) => (
          <div
            key={section.title}
            className="dot-matrix md:p-10 border-b-[0.5px] border-black/30 dark:border-white/30 "
          >
            <h3 className="text-2xl md:text-4xl font-semibold mb-4 dark:text-white text-black">
              {section.title}
            </h3>
            {section.content && (
              <p className="text-sm md:text-2xl font-extralight dark:text-white/70 text-black/80 mb-6 max-w-4xl">
                {section.content}
              </p>
            )}
            {section.items && (
              <ul className="grid mt-10 md:grid-cols-12 grid-col-8 gap-5">
                {section.items.map(({ heading, content, link }, index) => (
                  <Card
                    content={content}
                    heading={heading}
                    key={heading}
